...
palletPrice = Config.Palletprice
...
RegisterNetEvent('qb-burgershot:warehouseAccount', function()
    local src = source
    local balance = exports['qb-management']:GetAccount('burgershot')
    if balance >= palletPrice then-- bossmoney check
        exports['qb-management']:RemoveMoney('burgershot', palletPrice)
        TriggerClientEvent('qb-burgershotjob:CallWarehouseSuccess', source)
    else
        TriggerClientEvent('QBCore:Notify', source, 'There is no money in the company account..', 'error')
    end
end)